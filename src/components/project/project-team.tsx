  ...
  return (
    <div className="flex flex-col gap-5 w-full">
      <div>
        ...
        <p className="text-xs text-muted-foreground">Members assigned to collaborate on this project.</p>
      </div>
      ...
      {projectData.members.length > 0 ? (
        ...
        <div className="py-16 flex flex-col items-center justify-center bg-card/30 rounded-2xl border border-dashed border-border/80 text-center w-full">
          <UsersIcon className="size-10 text-muted-foreground/50 mb-3" />
          <p className="text-sm font-semibold text-foreground">No members assigned</p>
          <p className="text-xs text-muted-foreground mt-1">Use edit project to assign a team.</p>
        </div>
      )}
    </div>
  )
